...
The icons referenced in this version are a simple placeholder generated to match the app's own colour palette, a plain onion silhouette in the same green used throughout the interface. Anyone is welcome to replace them with a proper designed icon later without touching anything else in the manifest.
